...
foreach($policyFolder in $policyFolders)
{
    ...
    {
        $destructionBlock = {
            DeletePolicyAssignmentAndChildren -Name $policyFolder.Name -Scope $assignmentScope;
        }
    ...
    else
    {
        ...
        $policySetArgs["DisplayName"] = "[Initiative] - METADATA MISSING - $($policyFolder.Name)";
        $policySetArgs["Description"] = "[Initiative] - METADATA MISSING - $($policyFolder.Name)";
    }
