...
class PolicyDefinition : PolicyBase
{
	...
	[string]GetPolicySetParameterName([string]$localParameterName)
	{
		...
		if($this.GlobalParameterNames.Contains($localParameterName) -eq $true)
		{
...
class PolicyObject
{
	...
	PolicyObject(
		...
		{
			$this.AuditPolicy = [PolicyDefinition]::new($inputObject.policyObjects.audit, $this.Name + "_audit", $false, $this.Controls, $globalParameterNames, "audit", $policySetName);
			$hasAnyPolicies = $true;
		}

		if($inputObject.policyObjects.append -ne $null)
		{
			$this.AppendPolicy = [PolicyDefinition]::new($inputObject.policyObjects.append, $this.Name + "_append", $false, $this.Controls, $globalParameterNames, "append", $policySetName);
			$hasAnyPolicies = $true;
		}

		if($inputObject.policyObjects.deny -ne $null)
		{
			$this.DenyPolicy = [PolicyDefinition]::new($inputObject.policyObjects.deny, $this.Name + "_deny", $false, $this.Controls, $globalParameterNames, "deny", $policySetName);
			$hasAnyPolicies = $true;
		}

		if($inputObject.policyObjects.remediate -ne $null)
		{
			$this.RemediatePolicy = [PolicyDefinition]::new($inputObject.policyObjects.remediate, $this.Name + "_remediate", $true, $this.Controls, $globalParameterNames, "deployIfNotExists", $policySetName);
			$hasAnyPolicies = $true;
		}

		if($inputObject.policyObjects.guestConfigurationPrerequisites -ne $null)
		{
			$this.GuestConfigurationPrerequisites = [PolicyDefinition]::new($inputObject.policyObjects.guestConfigurationPrerequisites, $this.Name + "_gcPreReq", $true, $this.Controls, $globalParameterNames, "deployIfNotExists", $policySetName);
			$hasAnyPolicies = $true;
		}
	}
...
Function CreatePolicyDefinition
{
	...
	else
	{
        ...
        $existingDefinition = Get-AzPolicyDefinition -Name $Policy.Name -ErrorAction SilentlyContinue
...
Function AssignRequiredRoles
{
    ...
    foreach($roleDefinition in $Assignment.RoleDefinitionIds)
    {
		...
		$x = Get-AzRoleAssignment -ObjectId $Assignment.IdentityObjectId -Scope $Assignment.Scope -RoleDefinitionId $roleDefinitionId -Verbose:$VerbosePreference;

		if($x -ne $null)
		{
			LogVerbose "Found that role definition $($roleDefinition)@$($Assignment.Scope):$($Assignment.IdentityObjectId) already exists at this scope.";
		}
		else 
		{
			LogVerbose "Pulled '$roleDefinitionId' from '$roleDefinition'"

        	New-AzRoleAssignment -ObjectId $Assignment.IdentityObjectId -Scope $Assignment.Scope -RoleDefinitionId $roleDefinitionId -Verbose:$VerbosePreference | out-null;
		}
    }
}
